...
struct UIVertex {
    ...
    glm::vec2 uv;          // 0-1 within quad
    glm::vec4 shapeInfo;   // x=type(0=flat,1=rounded,2=ellipse), y=halfW, z=halfH, w=param
};
...
public:
    ...
    void setResource(VkDescriptorSet set, uint32_t dynamicOffset);
...
private:
    ...
    VkDescriptorSet  boundDescriptor_ = VK_NULL_HANDLE;
...
public:
    ...
    // Forward to arena for appending POD glyph/position data after params
    template <typename T>
